...
async function runDiagnostic() {
  ...
  try {
    ...
    console.error('  ❌ Meta rejected token:', err.response?.data?.error?.message || err.message);
    process.exit(1);
  }
  ...
  if (appId && appSecret) {
    try {
      ...
      console.log('  (Skipping token scope inspection:', err.response?.data?.error?.message || err.message, ')');
    }
  }
  ...
  try {
    ...
    console.error('\n  ❌ Text message failed:', metaError?.message || err.message);
    ...
    if (metaError?.code === 131030 || metaError?.code === 131047 || metaError?.error_subcode === 131030) {
      ...
      try {
        ...
        console.error('  ❌ Template send also failed:', tplErr.response?.data?.error?.message || tplErr.message);
      }
    }
